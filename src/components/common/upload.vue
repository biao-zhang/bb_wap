<template>
  <div>
    <div class="uploadCon">
      <div class="uploadItem" v-for="(item,index) in itemDatas" :key="index" :style="{'width':width,'height':width}">
      ...
      <div class="uploadItem" v-if="itemDatas.length<maxUploadNumber" @click="addItem" :style="{'width':width,'height':width}" >
        <div class="uploadItemImg" :style="{'backgroundImage':'url('+require('./img/addImg.png')+')'}" style="background-position: center;"></div>
      </div>
...
<style scoped>
  ...
  .uploadItem{display:inline-block;margin-right:0.1rem;background-color: white;border-radius: 0.05rem;border:1px solid #ccc;vertical-align: middle;font-size: 0.28rem;}
  ...
  .uploadItem .uploadItemImg .shadowBox .img2{width:30%;padding:0.04rem;cursor: pointer;position: absolute;top:-15%;right: -15%;}
