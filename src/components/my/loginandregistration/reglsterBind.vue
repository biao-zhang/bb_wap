<template>
  <div class='register'>
    ...
    <div class="biao">
      <div class="biao1">
        <label for="">昵称</label>
        <input type="text" placeholder="设置昵称" v-model="nickname">
      </div>
      <div class="biao1">
        <label for="">手机号</label>
      ...
      <div class="biao1">
        ...
        <input class="input1" type="text" placeholder="验证码" v-model="yzm" style="width:3rem;" /><span class="s2"><span v-if="!sendMsgDisabled" @click="send">发送验证码</span><span class="hs"  v-if="sendMsgDisabled">{{time}}后重新获取</span> </span>
      </div>
      <div class="biao1">
      ...
      <!--按钮-->
      <div class="btn" v-if="nickname!='' && phone!='' && pasw!='' && yzm!='' && radioState == true" @click="regBtn">注册并绑定</div>
...
//      发送验证码接口
      send(){
        ...
        this.axios.post('/personally/userRegister/sendCode',this.qs.stringify({
          phone:this.phone,
          type:1
        }))
          .then(function (response) {
            ...
            if(datas.message == 'success'){
              ...
              that.yzmdata = response.data.data;
            }else {
      ...
      regBtn(){
        ...
        if(this.authcode == false ||this.yzm != this.yzmdata){
          ...
          var that = this;
          this.axios.post('/personally/userRegister/addAndBindUser',this.qs.stringify({
