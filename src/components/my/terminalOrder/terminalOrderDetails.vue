  ...
  <div class='orderDetails'>
    ...
    <div class="orderDetailsList">
      <div class="orderDetailsListStatus">
        <div v-if="singleOrder.status === 0">其他状态</div>
        <div v-if="singleOrder.status === 1">已完成</div>
        <div v-if="singleOrder.status === 2">待付款</div>
        <div v-if="singleOrder.status === 3">待消费</div>
      ...
      <div class="orderDetailsListNum" style="font-size: 0.25rem">
        ...
        <div class="orderTime">
          <span>配送方式：</span>
          <!--是否是店铺配送 0： 店铺配送 1：上门自取-->
          <span>{{ singleOrder.isDsd === 0 ? '店铺配送' : '到店自取'}}</span>
        </div>
        <div class="receInfo" v-if="singleOrder.isDsd === 0">
          <span>收货信息：</span>
          <div>
            <div>{{ singleOrder.shipName }}   {{ singleOrder.shipMobile }} </div>
            <div style="margin-top: 0.10rem">{{ singleOrder.shippingArea }}</div>
          </div>
        </div>
      </div>
    ...
    <div v-transfer-dom class="popup_zong1">
      <popup v-model="show_kefu" position="bottom" max-height="70%">
        <!--客服-->
        <div class="kefu">
          <div class="kefu_top">
            <div>
...
<script>
  ...
  import { o2oGoodDetails } from "@/api/personally"
